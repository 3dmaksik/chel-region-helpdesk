...
<div class="card mb-4">
    ...
    <div class="card-body">
      <form id="formValidate" method="POST" action="{{ route(config('constants.work.update'),$items['items']->id) }}">
        @method('PATCH')
        @csrf
        <div class="form-group">
            <label for="">Фамилия</label>
            <input type="text" name="lastname" value="{{ $items['items']->lastname }}" class="form-control @error('lastname') is-invalid @enderror" id="lastname" aria-describedby="textLastname" placeholder="Фамилия" autocomplete="off">
        ...
        <div class="form-group">
            <label for="">Имя</label>
            <input type="text" name="firstname" value="{{ $items['items']->firstname }}" class="form-control @error('firstname') is-invalid @enderror" id="firstname" aria-describedby="textName" placeholder="Имя" autocomplete="off">
        ...
        <div class="form-group">
            ...
            <input type="text" name="patronymic" value="{{ $items['items']->patronymic }}" class="form-control @error('patronymic') is-invalid @enderror" id="patronymic" aria-describedby="textPatronymic" placeholder="Отчество" autocomplete="off">
        ...
        <div class="form-group">
            ...
            <select class="select2-single form-control" name="cabinet_id" id="select2-cabinet">
                ...
                <option @if ($cabinet->id==$items['items']->cabinet->id) selected @endif value="@if ($cabinet->id==$items['items']->cabinet->id) {{ $items['items']->cabinet->id }} @else {{ $cabinet->id }}@endif">@if ($cabinet->id==$items['items']->cabinet->id) {{ $items['items']->cabinet->description }} @else {{ $cabinet->description }}@endif</option>
                @endforeach
              </select>
        ...
        <div class="form-group">
            ...
            <select class="select2-single form-control" name="user_id" id="select2-users">
              ...
              <option @if ($user->id==$items['items']->user->id) selected @endif value="@if ($user->id==$items['items']->user->id) {{ $items['items']->user->id }} @else {{ $user->id }}@endif">@if ($user->id==$items['items']->user->id) {{ $items['items']->user->name }} @else {{ $user->name }}@endif</option>
              @endforeach
            </select>
