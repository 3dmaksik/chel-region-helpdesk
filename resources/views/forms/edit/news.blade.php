    ...
    <div class="card-body">
      <form id="formValidate" method="POST" action="{{ route(config('constants.news.update'),$item->id) }}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" value="{{ $item->name }}" class="form-control" id="name" aria-describedby="textName" placeholder="Название" autocomplete="off">
            <small id="textName" class="form-text text-muted">Введите название новости</small>
        </div>
        <div class="form-group">
            <label for="">Описание</label>
            <input type="text" name="description" value="{{ $item->description }}" class="form-control" id="description" aria-describedby="textDescription" placeholder="Описание" autocomplete="off">
        ...
        <div class="form-group">
            ...
            <textarea class="wysiwyg form-control" id="news-text" rows="9" name="news_text">{{ $item->news_text }}</textarea>
        </div>
        <div class="form-group" id="simple-date1">
            <label for="simpleDataInput">Дата и время создания</label>
              <div class="input-group date">
                ...
                <input id="datetimepicker" name="created_at" type="text" class="form-control" value="{{ $item->created_at }}" id="simpleDataInput">
              </div>
          </div>
