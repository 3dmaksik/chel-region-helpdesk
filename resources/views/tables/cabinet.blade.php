...
<div class="card">
	<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        ...
        <a href="{{ route(config('constants.cabinet.create')) }}"> <button type="button" class="btn btn-primary mb-1">Новый кабинет </button></a>
	</div>
			...
			<tbody id="table-dynamic">
				...
				<tr class="table-elements">
					...
					<td class="d-print-none">
						<div class="block">
                            <a href="{{ route(config('constants.cabinet.edit'),$item->id) }}" class="cabinet-edit btn btn-success btn-sm hover">
        ...
        <div class="modal fade" id="removeModal-{{$item->id}}" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form id="formRemove-{{$item->id}}" class="remove-form" action="{{ route(config('constants.cabinet.destroy'),$item->id) }}" method="POST">
                        @method('DELETE')
                        @csrf
                        <div class="modal-body">
                            <div class="form-group">
                            <p class="text-center">Вы действительно хотите удалить запись?</p>
                            <p class="text-center">Убедитесь, что у кабинета отсутствуют сотрудники</p>
                            </div>
                        </div>
